...
from comaze.agents.utils import dict_obs_discrete_action_secret_rule_extract_exp_fn, discrete_action_format_move_fn


class SimpleCommunicatingSecretRuleOnPolicyRLAgent(AbstractOnPolicyRLAgent):
  """
  Simple communicating on-policy RL agents using PyTorch, 
  able to account for the secret goal rule in later levels.
  ...
  def __init__(
    ...
    AbstractOnPolicyRLAgent.__init__(
      self=self,
      extract_exp_fn=dict_obs_discrete_action_secret_rule_extract_exp_fn, 
  ...
  def build_agent(self):
    ...
    self.embed_secret_rule = nn.Linear(8, self.embed_secret_rule_size)
    
    self.policy_input_size = self.embed_pov_size+self.embed_message_size+self.embed_action_size+self.embed_secret_rule_size
    self.policy = nn.Linear(self.policy_input_size, self.num_actions)
  ...
  def select_action(self, observation: Any) -> Dict[str, Any]:
    ...
    secret_rule_emb = self.embed_secret_rule(secret_rule)
    
    pov_message_action_secret_emb = torch.cat((pov_emb, message_emb, action_emb, secret_rule_emb), dim=1)
    action_pred = self.policy(pov_message_action_secret_emb)
